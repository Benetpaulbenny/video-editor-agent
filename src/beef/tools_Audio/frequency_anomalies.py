...
Part of the video-editor-agent project (src/beef/). Dependencies are managed
via the project's pyproject.toml / uv.lock, not inline script metadata.

Run with: uv run src/beef/detect_frequency_anomalies.py
"""
...
def detect_anomalies(
    ...
    rolling_window_frames: int = ROLLING_WINDOW_FRAMES,
    ...
    y, sr = librosa.load(str(in_path), sr=sample_rate, mono=True)
    ...
    dom_z = rolling_zscore(dom_delta, rolling_window_frames)
    cen_z = rolling_zscore(cen_delta, rolling_window_frames)

    combined_z = np.maximum(dom_z, cen_z)
    flagged = combined_z >= z_threshold

    print(f"Flagged {flagged.sum()} / {len(flagged)} frames above z={z_threshold}")
    ...
    if len(flagged_idx) > 0:
        ...
        for idx in flagged_idx[1:]:
            if times[idx] - times[prev] > merge_gap_sec:
    ...
    with open(out_path, "w", newline="") as f:
        ...
        for start_i, end_i in events:
            ...
            def window_avg(signal, center_start, center_end, before=False, after=False):
                if before:
                    mask = (times >= center_start - context_sec) & (times < center_start)
                elif after:
                    mask = (times > center_end) & (times <= center_end + context_sec)
                else:
